...
processFeed <- function (feedName, day, hour, columns) {
  ...
  sapply(files, FUN = function(file) {
    if (allHours || substr(file, 1, 13) == paste0(day, '_', ifelse(hour < 10, paste0('0', hour), hour))) {
      print(paste0('Reading file from disk - ',feedName, ': ', file))
      tsv <- suppressMessages(suppressWarnings(read_tsv(paste0(path, '/', file), col_names = colNames, progress = interactive())))
      data <<- rbind.data.frame(data, tsv[, outputColumns])
    }
  })
