...
summ <- summarise(gb,
                  clicks = sum(ifelse(event_type == 'click', 1, 0)),
                  impressions = sum(ifelse(event_type == 'imp', 1, 0))
                  )


saveRDS(summ, file = './test.RDS')
